...
class TestWordPressEmailVerification extends WP_UnitTestCase {
	...
	protected $verification_signature_key_name = 'testeleven_email_signature';

	public function setUp() {
		$this->email_verifier = new \Testeleven\WordPressEmailVerification\WordPressEmailVerification( 'testeleven_email_signature' );
	}
	...
	public function test_verify_email_after_password_reset_removes_email_flag_when_sigs_match() {
		$email_verification_sig = time() . ':' . '1234';
		$_POST['mail_key']   = $email_verification_sig;
		$user_id                = $this->factory->user->create();
		update_user_meta( $user_id, $this->verification_signature_key_name, $email_verification_sig );
	...
	public function test_verify_email_after_password_reset_doesnt_remove_flag_when_sigs_dont_match() {
		...
		$_POST['mail_key']   = $email_verification_sig;
		$user_id                = $this->factory->user->create();
		update_user_meta( $user_id, $this->verification_signature_key_name, 'differentkey' );
		$user   = get_userdata( $user_id );
	...
	public function test_verify_email_after_login_calls_process_expired_sig_when_sig_is_expired() {
		...
		$expired_verification_sig = $expired . ':' . '1234';
		$_POST['mail_key']      = $expired_verification_sig;
		$user_id                = $this->factory->user->create();
		$user                   = get_userdata( $user_id );
		update_user_meta( $user_id, $this->verification_signature_key_name, $expired_verification_sig );

		$wordpress_email_verification_mock = $this
			->getMockBuilder( '\Testeleven\WordPressEmailVerification\WordPressEmailVerification' )
			->setConstructorArgs( [ $this->verification_signature_key_name, 'testeleven_email_verification' ] )
	...
	public function test_verify_email_after_login_calls_process_mismatched_sig_when_sigs_mismatched() {
		$time = time();
		$email_verification_sig = $time . ':' . '1234';
		$mismatched_sig = $time . ':' . '5678';
		$_POST['mail_key'] = $email_verification_sig;
		$user_id = $this->factory->user->create();
		$user = get_userdata( $user_id );
		update_user_meta( $user_id, $this->verification_signature_key_name, $mismatched_sig );
	...
	public function test_verify_email_after_login_calls_process_mismatched_sig_when_timestamp_is_altered() {
		$time = time();
		$email_verification_sig = $time . ':' . '1234';
		$altered_sig = $time - 1 . ':' . '1234';
		$_POST['mail_key'] = $altered_sig;
		$user_id = $this->factory->user->create();
		$user = get_userdata( $user_id );
		update_user_meta( $user_id, $this->verification_signature_key_name, $email_verification_sig );
